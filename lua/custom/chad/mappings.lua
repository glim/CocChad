...
M.general = {
  n = {
    ...
    ["<leader>n"] = { "<CMD>cw<CR><CMD>cn<CR>zv", "Open quickfix list and jump to next entry or close if empty", opts =
    nowait },
    ["<leader>p"] = { "<CMD>cw<CR><CMD>cp<CR>zv", "Open quickfix list and jump to previous entry or close if empty", opts =
    nowait },
...
M.coc = {
  n = {
    ...
    ["<C-d>"] = { 'coc#float#has_scroll() ? coc#float#scroll(1) : "<C-d>"', "Scroll down", opts = vim.tbl_extend("error",
      silent, nowait, expr) },
    ["<C-u>"] = { 'coc#float#has_scroll() ? coc#float#scroll(0) : "<C-u>"', "Scroll up", opts = vim.tbl_extend("error",
      silent, nowait, expr) },
    ["<leader>[]"] = { ':<C-u>CocList diagnostics<CR>', "Show all diagnostics", opts = vim.tbl_extend("error", silent,
      nowait) },
    ["<leader>x"] = { ':<C-u>CocList extensions<CR>', "Manage extensions", opts = vim.tbl_extend("error", silent, nowait) },
    ["gs"] = { ':<C-u>CocList outline<CR>', "Goto the symbol of the current file", opts = vim.tbl_extend("error", silent,
      nowait) },
    ["<leader>fs"] = { ':<C-u>CocList -I symbols<CR>', "Find symbols", opts = vim.tbl_extend("error", silent, nowait) },
    ["<leader>j"] = { ':<C-u>CocNext<CR>', "Do default action for next item", opts = vim.tbl_extend("error", silent,
      nowait) },
    ["<leader>k"] = { ':<C-u>CocPrev<CR>', "Do default action for previous item", opts = vim.tbl_extend("error", silent,
      nowait) },
    ...
    ["<leader>ar"] = { '<Plug>(coc-codeaction-selected)', "Add codeaction to selected region", opts = vim.tbl_extend(
    "error", silent, nowait) },
    ["<leader>ag"] = { '<Plug>(coc-codeaction)', "Apply code action to current buffer", opts = vim.tbl_extend("error",
      silent, nowait) },
    ["<leader>fl"] = { '<Plug>(coc-fix-current)', "Autofix current line", opts = vim.tbl_extend("error", silent, nowait) },
    ["<leader>ll"] = { '<Plug>(coc-codelens-action)', "Perform CodeLens action on the current line", opts = vim
    .tbl_extend("error", silent, nowait) },
    ...
    ["<leader>sfE"] = { ':call jobstart("Emulator @flutter_emulator -no-snapshot-load")<CR>',
      "cold start flutter_emulator", opts = silent },
    ...
    ["<leader>sfr"] = { '<CMD>CocCommand flutter.run<CR><CMD>CocCommand flutter.dev.hotReload<CR>',
      "Run / hot reload Flutter app on device/emulator", silent },
    ...
    ["<leader>sfR"] = { '<CMD>CocCommand flutter.dev.clearDevLog<CR><CMD>CocCommand flutter.dev.hotRestart<CR>',
      "Hot restart Flutter dev server" },
    ...
    ["<leader>sfp"] = { '<CMD>CocCommand flutter.dev.showPerformanceOverlay<CR>',
      "Show Flutter dev server performance overlay" },
    ...
    ["<leader>sdp"] = { '<CMD>CocCommand deno.performance<CR>',
      "Display the timing averages for the internal Deno instrumentation" },
  ...
  i = {
    ...
    ["<C-u>"] = { 'coc#float#has_scroll() ? coc#float#scroll(0) : "<C-u>"', "Scroll up", opts = vim.tbl_extend("error",
      silent, nowait, expr) },
    ["<C-n>"] = { 'coc#pum#visible() ? coc#pum#next(1) : v:lua.check_back_space() ? "<C-n>" : coc#refresh()',
      "Move suggestion down / show suggestions", opts = vim.tbl_extend("error", silent, noremap, expr,
      no_replace_keycodes) },
    ["<Down>"] = { 'coc#pum#visible() ? coc#pum#next(1) : "<Down>"', "Move suggestion down", opts = vim.tbl_extend(
    "error", silent, noremap, expr, no_replace_keycodes) },
    ["<C-p>"] = { 'coc#pum#visible() ? coc#pum#prev(1) : "<C-p>"', "Move suggestion up", opts = vim.tbl_extend("error",
      silent, noremap, expr, no_replace_keycodes) },
    ["<Up>"] = { 'coc#pum#visible() ? coc#pum#prev(1) : "<Up>"', "Move suggestion up", opts = vim.tbl_extend("error",
      silent, noremap, expr, no_replace_keycodes) },
    ["<CR>"] = { 'coc#pum#visible() ? coc#pum#confirm() : "<CR>"', "Confirm selection", opts = vim.tbl_extend("error",
      silent, noremap, expr, no_replace_keycodes) },
  ...
  x = {
    ["<leader>if"] = { '<Plug>(coc-funcobj-i)', "Map insert function object", opts = vim.tbl_extend("error", silent,
      nowait) },
    ["<leader>af"] = { '<Plug>(coc-funcobj-a)', "Map append function object", opts = vim.tbl_extend("error", silent,
      nowait) },
    ...
    ["<leader>a"] = { '<Plug>(coc-codeaction-selected)', "Add codeaction to selected region", opts = vim.tbl_extend(
    "error", silent, nowait) },
    ["<leader>rn"] = { '<Plug>(coc-rename)', "Refactor rename symbol", opts = silent },
    ["<C-y>"] = { '<Plug>(coc-range-select)', "Select ranges", opts = silent },
  },
...
M.arduino = {
  n = {
    ...
    ["<leader>as"] = { "<CMD>ArduinoSerial<CR>", "Connect to the Arduino board for debugging over a serial port", opts =
    silent },
    ["<leader>aus"] = { "<CMD>ArduinoUploadAndSerial<CR>", "Build, upload the Arduino sketch, and connect for debugging", opts =
    silent },
